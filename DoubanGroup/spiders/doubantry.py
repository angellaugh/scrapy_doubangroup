...
5. 遗留，title  中 包含 /  导致103行写入时，报错；
6. 遗留：删除回帖中的部分冗余字段回应，删除，赞；
7. 遗留：保存图片url；
...
class DoubanSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for detail in s.css('td.title'):
            ...
            theme_link += a
            print(theme_link)
            yield scrapy.Request(url=theme_link,callback=self.parse_detail,cookies=cookies,headers=headers)
        # 通过next_page 来翻页
    ...
    def parse_detail(self,response):
        ...
        f = open("/Users/vivi/PycharmProjects/DoubanGroup/FullGroup/%s.txt" % title1, "a")
        print(ddata, file=f)
        f.close()

        # 暂时用不到replay1 和 replay2
        # replay1 是所有最赞回应
